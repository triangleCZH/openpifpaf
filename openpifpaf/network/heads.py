...
class CPMAFF(nn.Module):
    def __init__(self, input_features = 1024,num_refinement_stages=1, num_channels=128, num_heatmaps=19, num_pafs=38, nettype='cpmaff'):
        ...
        for idx in range(num_refinement_stages):
            self.refinement_stages.append(CPMRefinementStage(input_channel, num_channels,
...
class CompositeField(torch.nn.Module):
    ...
    def __init__(self, head_name, in_features, *,
                 ...
                 n_vectors,
                 n_scales,
                 kernel_size=1, padding=0, dilation=1, initial_stage=False):
        ...
        self.initial_stage = initial_stage # for comaff
...
#         print("TRIANGLE before heads", x.shape)
        x = self.dropout(x)
        ...
        if self.initial_stage:
            for _ in range(self._quad):
                classes_x1 = [self.dequad_op(class_x)[:, :, :-1, :-1]
                             for class_x in classes_x]
                regs_x1 = [self.dequad_op(reg_x)[:, :, :-1, :-1]
                          for reg_x in regs_x]
                regs_x_spread1 = [self.dequad_op(reg_x_spread)[:, :, :-1, :-1]
                                 for reg_x_spread in regs_x_spread]
                scales_x1 = [self.dequad_op(scale_x)[:, :, :-1, :-1]
                            for scale_x in scales_x]

            # reshape regressions
            regs_x1 = [
                reg_x1.reshape(reg_x1.shape[0],
                              reg_x1.shape[1] // 2,
                              2,
                              reg_x1.shape[2],
                              reg_x1.shape[3])
                for reg_x1 in regs_x1
            ]
            return classes_x + regs_x + regs_x_spread + scales_x, classes_x1 + regs_x1 + regs_x_spread1 + scales_x1
        else:
            for _ in range(self._quad):
...
#         print("TRIANGLE after heads", classes_x[-1].shape, regs_x[-1].shape, regs_x_spread[-1].shape)
...
def factory(name, n_features, initial_stage=False):
    if name in ('pif',
                'paf',
                'pafs',
                'wpaf',
